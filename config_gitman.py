...
def modificar_odoo_conf():
    ...
    try:
        ...
        if not os.access(odoo_conf_path, os.R_OK):
            raise PermissionError(f"No se puede leer el archivo {odoo_conf_path}. Verifica los permisos.")
        if not os.access(odoo_conf_path, os.W_OK):
            raise PermissionError(f"No se puede escribir en el archivo {odoo_conf_path}. Verifica los permisos.")
        ...
        nombres_repositorios = [repo['name'] for repo in gitman_data['sources'] if repo['name']]
        ...
        nuevas_rutas = ",".join([f"/usr/lib/python3/dist-packages/odoo/external_addons/{nombre}" for nombre in nombres_repositorios])
        ...
        for i, line in enumerate(lines):
            if line.startswith("addons_path ="):
                ...
                rutas_actualizadas = lineas_rutas_existentes + [ruta for ruta in nuevas_rutas.split(",") if ruta not in lineas_rutas_existentes]
