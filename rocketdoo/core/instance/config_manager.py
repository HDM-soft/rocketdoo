...
class InstanceConfigManager:
    ...
    def _wizard_env(self, env: str) -> dict:
        ...
        if auth_method == 'ssh_key':
            ...
            if not available_keys:
                ...
                selected = questionary.select(
                    'Select the SSH key configured on this VPS:',
            ...
            console.print(
                '[dim]Password will be requested at deploy time '
                'or read from the env var below.[/dim]'
            )
            env_var = f'INSTANCE_{env.upper()}_PASSWORD'
            password_ref = Prompt.ask(
        ...
        gitman_config = ''
        if use_gitman:
            gitman_config = Prompt.ask(
                'Gitman config file (local path)',
                default=f'gitman.{env}.yml'
            )
