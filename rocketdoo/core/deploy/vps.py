...
class VPSDeployer(BaseDeployer):
    ...
    def pre_deploy_check(self) -> bool:
        ...
        try:
            ...
            if self.deployment_type == "native" and self.post_deploy_config.get("restart_service", False):
        ...
        try:
            ...
            with Progress(
                ...
                for module in modules:
                    ...
                    if not success:
                        return DeploymentResult(success=False, message=f"Failed to upload module: {module_name}")
            ...
            return DeploymentResult(
                success=True, message="Modules deployed successfully", details={"modules": [m["name"] for m in modules]}
            )

        except Exception as e:
            self.log(f"Deployment failed: {e}", "error")
            return DeploymentResult(success=False, message=f"Deployment error: {e}")
        ...
        try:
            ...
            if self.post_deploy_config.get("restart_service", False):
                ...
                if self.deployment_type == "docker":
                    ...
                    result = self._run_ssh_command(f"cd {self.compose_path} && docker-compose restart {self.container_name}")

                    if result.returncode != 0:
                        self.log(f"Failed to restart container: {result.stderr}", "error")
                        return DeploymentResult(success=False, message="Failed to restart Docker container")
                else:
                    # Restart systemd service with sudo (handles password if needed)
                    result = self._run_ssh_command(f"systemctl restart {self.service_name}", use_sudo=True)

                    if result.returncode != 0:
                        self.log(f"Failed to restart service: {result.stderr}", "error")
                        return DeploymentResult(success=False, message="Failed to restart Odoo service")
            ...
            if self.post_deploy_config.get("update_modules", False):
                ...
                if self.deployment_type == "docker":
                    ...
                    update_cmd = f"docker exec {self.container_name} odoo-bin -c /etc/odoo/odoo.conf -u all --stop-after-init"
                else:
                    # Update via direct command
                    python_env = self.native_config.get("python_env", "python3")
                    update_cmd = (
                        f"{python_env} {self.odoo_path}/odoo-bin -c {self.odoo_path}/odoo.conf -u all --stop-after-init"
                    )
            ...
            return DeploymentResult(success=True, message="Post-deploy actions completed")

        except Exception as e:
            self.log(f"Post-deploy actions failed: {e}", "error")
            return DeploymentResult(success=False, message=f"Post-deploy error: {e}")
    ...
    def _run_ssh_command(self, command: str, timeout: int = 300, use_sudo: bool = False) -> subprocess.CompletedProcess:
        ...
        # Add SSH options
        ssh_cmd.extend(["-o", "StrictHostKeyChecking=no", "-o", "UserKnownHostsFile=/dev/null", "-p", str(self.port)])
    ...
    def _upload_file_scp(self, local_path: Path, remote_path: str) -> bool:
        ...
        try:
            ...
            # Add SSH options
            scp_cmd.extend(["-o", "StrictHostKeyChecking=no", "-P", str(self.port)])
    ...
    def rollback(self) -> DeploymentResult:
        ...
        try:
            ...
            if not backup_path.exists():
                return DeploymentResult(success=False, message="No backups found for rollback")
            ...
            if not backups:
                return DeploymentResult(success=False, message="No backups found for this target")
            ...
            for module_dir in latest_backup.iterdir():
                if module_dir.is_dir():
                    success = self._upload_directory(module_dir, f"{target_path}/{module_dir.name}")
                    if not success:
            ...
            return DeploymentResult(success=True, message="Rollback completed successfully")

        except Exception as e:
            self.log(f"Rollback failed: {e}", "error")
            return DeploymentResult(success=False, message=f"Rollback error: {e}")
