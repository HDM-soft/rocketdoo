...
def _traefik_running() -> bool:
    r = subprocess.run(
        ['docker', 'ps', '-q', '--filter', 'name=traefik'],
...
@click.option('--traefik-dir', default=_DEFAULT_TRAEFIK_DIR, show_default=True,
              help='Directory for the shared Traefik service')
def traefik_on(domain, mode, traefik_dir):
    ...
    # ── Mode ──
    if not mode:
        mode = existing.get('mode') or questionary.select(
            'Deployment mode:',
            choices=[
                questionary.Choice(
                    'Local dev  — HTTP, custom domain via /etc/hosts',
                    value='local'
                ),
                questionary.Choice(
                    'Production — HTTPS with Let\'s Encrypt certificate',
                    value='production'
                ),
            ],
            style=_custom_style
...
def traefik_off():
    ...
    console.print()
    console.print(Panel(
        '[bold cyan]Disabling Traefik[/bold cyan]',
        border_style='cyan', box=box.ROUNDED
    ))
    console.print()
...
def traefik_status():
    ...
    table.add_row(
        'Project override',
        '[green]Active[/green]' if override else '[yellow]Not configured — run rkd traefik on[/yellow]'
    )
    table.add_row(
        f'Network "{_NETWORK}"',
        '[green]Exists[/green]' if network else '[red]Missing[/red]'
    )
    table.add_row(
        'Traefik container',
    ...
    if config:
        ...
        table.add_row(
            'URL',
            f'[cyan underline]{scheme}://{domain}[/cyan underline]' if domain != '—' else '—'
        )
    ...
    console.print(Panel(
        table,
        title='[bold cyan]Traefik Status[/bold cyan]',
...
def traefik_guide():
    ...
    if is_wsl:
        ...
        console.print(
            f'  [green]Add-Content -Path \'{win_hosts}\' '
            f'-Value \'127.0.0.1  {domain}\'[/green]\n'
        )
