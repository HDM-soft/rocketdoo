...
def list_modules(ctx, path, show_all):
    ...
    for module in display_modules:
        ...
        table.add_row(
            f"[{status_style}]{status}[/{status_style}]", name, module.version, str(module.relative_path), depends_str or "-"
        )
...
def deploy_config(ctx, edit):
    ...
    console.print(
        Panel(
            f"[bold cyan]📋 Deploy Configuration[/bold cyan]\n\n[dim]File:[/dim] {config_manager.config_path}",
            border_style="cyan",
            box=box.ROUNDED,
        )
    )
    ...
    for target_name, target_config in targets.items():
        ...
        table.add_row(f"[{status_style}]{status}[/{status_style}]", target_name, target_type, destination)
...
def deploy_run(ctx, target, module, dry_run, skip_backup, skip_validation, yes):
    ...
    console.print(
        Panel(
            f"[bold cyan]🚀 Deploying to: {target}[/bold cyan]\n\n[dim]Type:[/dim] {target_config.get('type', 'unknown')}",
            border_style="cyan",
            box=box.ROUNDED,
        )
    )
    ...
    scanner = ModuleScanner(addons_path, exclude_patterns=config.get("modules", {}).get("exclude_patterns"))
    ...
    try:
        ...
        if dry_run:
            ...
            if result.success:
                ...
                console.print(
                    Panel(
                        f"[bold red]❌ Deployment failed[/bold red]\n\n{result.message}", border_style="red", box=box.ROUNDED
                    )
                )
...
def validate_modules(ctx, path):
    ...
    console.print()
    console.print(Panel("[bold cyan]🔍 Validating Modules[/bold cyan]", border_style="cyan", box=box.ROUNDED))
