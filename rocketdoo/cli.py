...
@click.option("-i", "--identifier", "delete_identifier", is_flag=True, help="Delete .Identifier files generated by WSL2")
@click.option("--dry-run", is_flag=True, help="Simulate deletion without removing files")
@click.option(
    "-p", "--path", default=".", type=click.Path(exists=True), help="Directory to search (default: current directory)"
)
@click.option("-q", "--quiet", is_flag=True, help="Do not show each processed file")
@click.pass_context
def delete_command(ctx, delete_identifier, dry_run, path, quiet):
    ...
    # Execute cleanup
    deleted_count, errors = find_and_delete_identifiers(path, dry_run=dry_run, verbose=verbose)
...
@click.version_option(
    version=f"{__version__}", prog_name="Rocketdoo"
)  # version declared on rocketdoo/core/__init__.py and pyproject.toml
...
def info():
    ...
    if project_info["restart_policy"]:
        restart_emoji = {"no": "🚫", "always": "🔄", "unless-stopped": "⏸️", "on-failure": "⚠️"}.get(
            project_info["restart_policy"], "♻️"
        )
    ...
    if project_info["use_third_party_repos"] and project_info["third_party_repos"]:
        repos_table = Table(show_header=True, box=box.SIMPLE_HEAD, padding=(0, 1), expand=True)
        ...
        for repo in project_info["third_party_repos"]:
            ...
            repos_table.add_row(repo["name"], repo_url, repo["rev"])
    ...
    if missing:
        ...
        console.print(
            Panel(
                warn,
                title="[bold yellow]Secrets at risk[/bold yellow]",
                border_style="yellow",
                box=box.ROUNDED,
                padding=(1, 1),
            )
