...
def _backup_filestore(odoo_container: str, db_name: str, output_path: Path) -> tuple[bool, str | None]:
    """
    Copies and compresses the Odoo filestore from the container to the host.
    Returns (success: bool, filestore_base: str | None).
        ...
        if not db_container:
            ...
            if not available_dbs:
                ...
                if odoo_container and _is_container_running(odoo_container):
                    fs_ok, filestore_base = _backup_filestore(odoo_container, selected_db, fs_backup_path)
                    if not fs_ok:
                        fs_backup_path = None
                else:
